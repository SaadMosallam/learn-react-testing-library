...
import { employees } from "src/utils";
...
describe("Table tests", () => {
  test("Table component rendered correctly", () => {
    ...
    expect(
      screen.getByRole("cell", { name: /Christina Smith/i })
    ).toBeInTheDocument();
    const roleCells = screen.getAllByRole("cell", { name: /Frontend/i });
    const departmentCells = screen.getAllByRole("cell", {
      name: /Engineering/i,
    });
  ...
  test("Table component styled correctly", () => {
    ...
    expect(table).toHaveAttribute(
      "class",
      expect.stringContaining("table-striped")
    );
  });
});
